...
- Prototype Settings now includes a Persistence diagnostics panel showing redacted identity, desired-versus-active infrastructure composition, identity safety status, snapshot schema/health, journal counts, last recovery/migration, UGS preflight status, Remote Config safety status, disabled profile/settings Cloud Save sync status, blocked conflict/offline policy gates, local journal sync-staging policy, journal retention/privacy policy, and diagnostics export/share safety policy. It can export a payload-free safe summary as a local file only; in-app share, clipboard forwarding, and external Beta share flows remain blocked until policy is approved. Ordered snapshot migrations are supported; journal retention/deletion remains disabled until policy is approved.
...
- A second UGS runtime gate scaffold now exists for future Authentication/Analytics/Cloud Save adapters: even with `BMM_UGS_ADAPTERS` compiled, live calls stay blocked unless explicit runtime approvals for project link, environment, services initialization, consent, and per-service enablement are passed. The local composition root still does not construct any live UGS adapters.
- The composition root can now accept future provider preferences for identity, analytics, and profile/settings cloud sync while still reporting desired versus active providers through diagnostics. In the current build, requested UGS-backed providers safely fall back to local-first providers when the compile gate is absent or runtime approvals are blocked.
- Provider selection is now being centralized through an infrastructure provider/factory layer so local-first defaults and future UGS-backed overrides can be staged in one place, while adapter construction remains isolated behind the UGS boundary instead of spreading across the root service assembly.
...
- Analytics safety policy `analytics_safety_v0.1` keeps consent and live upload blocked, tracks allowlisted vs local-only analytics events, and blocks Remote Config-driven upload bypass. The local-only instrumentation surface now records safe envelopes for room enter, round start, bingo claim, round completion, round reward collect, room restore, album reward claim, social freebie redeem, social help request send, friend mana send/receive, inbox actions, daily bonus claim, daily spin claim, coven orb contribution, wild-card use, coven wish gifting, coven emporium purchases, and jackpot collect.
- Analytics payload/schema construction now routes through `Assets/Scripts/Infrastructure/PrototypeAnalyticsPayloadFactory.cs` for reusable local event shaping, and `Bingo Magic Mayhem/Docs/18_LOCAL_ANALYTICS_EVENT_CATALOG.md` documents the current allowlisted event surface and exclusions.
...
- Diagnostics export/share safety policy `diagnostics_export_safety_v0.1` keeps support exports local-file-only, payload-free, and explicit while leaving Remote Config non-authoritative for disabling the local support export path.
- Unity EditMode `InfrastructureServiceTests` last passed 40/40 on 2026-07-10 after the final local-only analytics coverage pass. The composition-scaffold pass adds three more EditMode tests for the next Unity Test Runner pass.
