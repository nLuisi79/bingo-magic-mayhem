...
- Analytics safety policy `analytics_safety_v0.1` keeps consent and live upload blocked, tracks allowlisted vs local-only analytics events, and blocks Remote Config-driven upload bypass. The first feature-level local-only instrumentation pass now records safe envelopes for room enter, round start, bingo claim, round completion, daily bonus claim, daily spin claim, inbox reward claim, inbox message read, and inbox bulk clear.
