...
- The approved UGS package entries are resolved in `Packages/manifest.json`, `Packages/packages-lock.json`, and the Unity package cache. Live adapters are compile-gated behind `BMM_UGS_ADAPTERS` and remain disabled; the local preflight now reports package, define, project-link, consent, Cloud Save policy, and gameplay/economy sync readiness. Local journal policy classifies safe future-upload candidates versus sensitive/unapproved rows, but active upload eligibility remains 0 and no upload path is active.
