...
- Analytics payload/schema construction has started moving into `Assets/Scripts/Infrastructure/PrototypeAnalyticsPayloadFactory.cs` so the prototype script only owns event timing and UI flow, while infrastructure owns reusable local analytics payload shaping.
