...
- The approved UGS package entries are resolved in `Packages/manifest.json`, `Packages/packages-lock.json`, and the Unity package cache. Live adapters are compile-gated behind `BMM_UGS_ADAPTERS` and remain disabled; the local preflight now reports package, define, project-link, consent, Cloud Save policy, and gameplay/economy sync readiness. The profile/settings Cloud Save scaffold uses key `bmm.profile_settings.v2` but reports upload/download blocked until project link, consent/privacy, conflict policy, and offline fallback are approved. Local journal policy classifies safe future-upload candidates versus sensitive/unapproved rows, but active upload eligibility remains 0 and no upload path is active.
- Unity EditMode `InfrastructureServiceTests` passed 12/12 after the expected-log assertion fix for newer snapshot rejection.
