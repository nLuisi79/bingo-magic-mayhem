...
- Trigger round reward collect, room restore, album reward claim, social freebie redeem, ingredient help request send, friend mana send/receive, coven orb contribution, wild-card use, coven wish gifting, coven emporium purchase, and jackpot collect, then confirm they also appear as allowlisted local analytics events with safe normalized context only.
- Confirm the helper-backed analytics payload path still omits message body text, keeps social payloads free of chat/message text, and continues using reusable infrastructure schemas for room context, inbox reward shaping, and the local analytics event catalog.
...
- Confirm diagnostics composition reporting shows desired versus active providers, and that any requested UGS-backed identity/analytics/profile-sync providers still fall back to local-first providers in the current build.
...
- Open Prototype Settings > Persistence and confirm the composition section shows desired versus active providers, plus explicit local-fallback reporting when UGS-backed providers are requested in configuration.
